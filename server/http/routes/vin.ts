...
/**
 * GET /proxy/:vin
 * Simple CORS-free proxy to vPIC API for direct browser calls
 */
router.get('/proxy/:vin', async (req, res) => {
  try {
    const rawVin = String(req.params.vin || '');
    const vin = normalizeVIN(rawVin);
    
    if (!isValidVIN(vin)) {
      return res.status(400).json({ 
        ok: false, 
        error: { message: 'Invalid VIN format' } 
      });
    }

    // Direct proxy to vPIC API
    const response = await fetch(
      `https://vpic.nhtsa.dot.gov/api/vehicles/DecodeVin/${vin}?format=json`,
      { 
        headers: { 'User-Agent': 'VINtage-Garage/1.0' }
      }
    );
    ...
    if (!response.ok) {
      return res.status(502).json({ 
        ok: false, 
        error: { message: `vPIC API error: ${response.status}` } 
      });
    }
    
    const data = await response.json();
    
    // Transform vPIC response to normalized format
    const results = (data?.Results || []).reduce((acc: any, row: any) => {
    ...
    console.error('[VIN proxy] error:', error);
    return res.status(500).json({ 
      ok: false, 
      error: { message: 'VIN proxy service failed' } 
    });
  }
});
